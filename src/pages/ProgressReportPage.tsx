...
export default function ProgressReportPage() {
  ...
  }, [yearMonth, workouts, allExercises])
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="px-4 flex-1">
        {report.trim() ? (
          ...
          <div className="flex flex-col items-center justify-center h-48 gap-2 text-gray-500"><p>Nema podataka za ovaj mesec</p></div>
        )}
      </div>
      {report.trim() && (
        <div className="px-4 py-6 flex flex-col gap-3">
          <button className="w-full py-3 rounded-xl bg-green-700 font-semibold flex items-center justify-center gap-2" onClick={() => window.open(`https://wa.me/${tid()}?text=${encodeURIComponent(report)}`, '_blank')}>
          ...
          <button className={`w-full py-3 rounded-xl font-semibold ${copied ? 'bg-green-800 text-green-200' : 'bg-gray-800 text-gray-300'}`} onClick={async () => { await navigator.clipboard.writeText(report); setCopied(true); setTimeout(() => setCopied(false), 2000) }}>
