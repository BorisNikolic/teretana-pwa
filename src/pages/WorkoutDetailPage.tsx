...
export default function WorkoutDetailPage() {
  ...
  const load = async () => {
    if (!workoutId) return
    setWorkout((await getWorkouts()).find(w => w.id === workoutId) ?? null)
  ...
  const handleAdd = async (data: ExerciseFormData) => {
    if (!workoutId) return
    const ex = await addExercise({ workoutId, name: data.name, order: exercises.length, type: data.type, setsCount: data.setsCount, reps: data.reps, restSeconds: data.restSeconds, notes: data.notes })
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const oi = exercises.findIndex(e => e.id === active.id), ni = exercises.findIndex(e => e.id === over.id)
    const reordered = arrayMove(exercises, oi, ni); setExercises(reordered)
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <div className="flex items-center gap-3 px-4 pt-4 pb-4">
        <button className="text-blue-400 p-1 -ml-1" onClick={() => navigate('/')}><svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg></button>
