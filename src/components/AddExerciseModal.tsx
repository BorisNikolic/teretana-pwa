...
export default function AddExerciseModal({ onSave, onClose }: Props) {
  ...
  const save = () => onSave({ name: name.trim(), type, setsCount, reps, restSeconds, notes, videoBlob })
      ...
      <div className="bg-gray-900 w-full rounded-t-2xl p-6 pb-10 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <button
              className={`flex-1 py-2.5 rounded-xl font-medium text-sm ${type === 'strength' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400'}`}
              onClick={() => setType('strength')}
            >
              Tegovi
            </button>
            <button
              className={`flex-1 py-2.5 rounded-xl font-medium text-sm ${type === 'cardio' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400'}`}
          ...
          <input
            ...
            placeholder={type === 'cardio' ? 'Naziv (npr. Trčanje)' : 'Naziv vežbe'}
            value={name}
          ...
          {type === 'strength' ? (
            ...
            <p className="text-sm text-gray-500 px-1">Vreme, brzinu i nagib unosiš na samom treningu.</p>
          )}
        ...
        <div className="flex gap-3 mt-4">
          ...
          <button className="flex-1 py-3 rounded-xl bg-blue-600 font-semibold disabled:opacity-40" disabled={!name.trim()} onClick={save}>
            Sačuvaj
          </button>
        </div>
      </div>
