...
export default function SessionSummary({ workoutId, onClose }: Props) {
  ...
  useEffect(() => {
    Promise.all([getAssignedWorkouts(), getExercises(workoutId)]).then(([ws, exs]) => {
      const name = ws.find(w => w.id === workoutId)?.name ?? ''
      getSessionSummary(name, exs).then(setText)
    })
  }, [workoutId])

  const sendWhatsApp = () => { window.open(`https://wa.me/${tid()}?text=${encodeURIComponent(text)}`, '_blank') }
